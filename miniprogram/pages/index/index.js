...
Page({
  ...
  async loadData(forceRefresh = false) {
    ...
    try {
      ...
      // 获取分类列表
      console.log('[index] 调用 category.list...')
      const { result: catResult } = await wx.cloud.callFunction({
        name: 'category',
        data: { action: 'list' }
      })
      console.log('[index] category.list 成功')
